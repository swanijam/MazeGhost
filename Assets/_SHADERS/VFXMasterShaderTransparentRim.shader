Shader "VFX/VFXMasterShaderTransparentRim"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                //Rim
                float rimDot = dot(normalize(i.viewDir), i.worldNormal);
                float rim = smoothstep(_RimSize, 0, saturate(rimDot));
                float4 colorRim = _RimColor * rim;

                //Transparency
                float cutoff = saturate(_Cutoff + (1 - i.color.a) - rim);
                float alpha = smoothstep(cutoff, cutoff + _CutoffSoftness, orCol) * colorRim;
